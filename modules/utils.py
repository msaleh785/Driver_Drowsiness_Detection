...
def draw_status_on_frame(frame, drowsiness_level, detailed_status, fps):
    ...
    # Show MAR and yawn status with frequency/timer - RED for visibility
    yawn_status = detailed_status.get('yawn_status', {})
    if yawn_status:
        # Get current MAR value and calculate percentage
        current_mar = yawn_status.get('mar', 0.0)
        mar_threshold = 0.6  # Must match threshold in yawn_detector.py
        
        # Calculate MAR percentage relative to threshold
        # Show percentage of how close to yawn threshold
        # Below threshold: show relative to threshold, above: 100%+
        mar_percentage = min(150, int((current_mar / mar_threshold) * 100))
        
        yawn_level = yawn_status.get('drowsiness', 'none')
        is_yawning = yawn_status.get('is_yawning', False)
        yawn_duration = yawn_status.get('yawn_duration', 0)
        yawn_frequency = int(yawn_status.get('yawn_frequency', 0))
        
        if yawn_level == 'high':
            yawn_percent = 100
        elif yawn_level == 'medium':
            yawn_percent = 70
        elif yawn_level == 'low-medium':
            yawn_percent = 50
        elif yawn_level == 'low':
            yawn_percent = 40
        else:
        ...
        # Always show MAR info with yawn count and timer - RED color
        mar_text = f"MAR: {current_mar:.3f} ({mar_percentage}%) | Yawns: {yawn_frequency}/min"
        if is_yawning and yawn_duration > 0:
            mar_text += f" | Duration: {yawn_duration:.1f}s"
        
        cv2.putText(frame, mar_text, (10, 100), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
        
        # Show yawn status if actively yawning or recently yawned - RED color
        if yawn_percent > 0 or is_yawning:
            yawn_text = f"Yawning: {yawn_percent}%" if yawn_percent > 0 else "Yawning: Detected"
            cv2.putText(frame, yawn_text, (10, 125), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
    
    # Show combined drowsiness score prominently - RED color
    scores = detailed_status.get('scores', {})
    if scores:
        total_s = scores.get('total', 0.0)
        total_percent = int(total_s * 100)
        
        # Always use RED for high visibility
        score_color = (0, 0, 255)  # RED
        
        cv2.putText(frame, f"Combined Score: {total_percent}% ({total_s:.2f})", 
                    (10, 175), cv2.FONT_HERSHEY_SIMPLEX, 0.7, score_color, 2)
    
    # Show EAR score (Eye Aspect Ratio) - RED color for visibility
    eye_status = detailed_status.get('eye_status', {})
    if eye_status:
        ear_left = eye_status.get('left_ear', 0.0)
        ear_right = eye_status.get('right_ear', 0.0)
        ear_avg = eye_status.get('avg_ear', (ear_left + ear_right) / 2.0)
        
        cv2.putText(frame, f"EAR: L={ear_left:.3f} R={ear_right:.3f} Avg={ear_avg:.3f}", 
                    (10, 200), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
    
    # Show final calculation result only - RED color
    if scores:
        eye_s = scores.get('eye', 0.0)
        yawn_s = scores.get('yawn', 0.0)
        head_s = scores.get('head', 0.0)
        
        # Show weights and final result
        result_text = f"Weights: E:60% Y:25% H:15% | Scores: E:{eye_s:.2f} Y:{yawn_s:.2f} H:{head_s:.2f}"
        cv2.putText(frame, result_text, (10, 225), 
