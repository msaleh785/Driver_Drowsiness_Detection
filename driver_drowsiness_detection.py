#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# conda activate ddd; python .\driver_drowsiness_detection.py --camera 0 --show_landmarks
# conda activate ddd; python .\driver_drowsiness_detection.py --camera 0 --show_landmarks --include_head
"""
...
parser.add_argument('--output', type=str, help='Path to output video file (e.g., output.mp4)')
...
parser.add_argument('--include_head', action='store_true', help='Include head movement detection in drowsiness analysis')
...
class DriverDrowsinessDetection:
    def __init__(self, args, config):
        ...
        if args.record or args.output:
            fourcc = cv2.VideoWriter_fourcc(*'mp4v') if args.output and args.output.endswith('.mp4') else cv2.VideoWriter_fourcc(*'XVID')
            ...
            if args.output:
                ...
                output_path = f'recordings/drowsiness_{timestamp}.avi'
                ...
                try:
                    ...
                    if face_landmarks:
                        ...
                        try:
                            ...
                            # Head monitoring is optional based on --include_head flag
                            if args.include_head:
                                head_status = self.head_monitor.process(face_landmarks)
                            else:
                                # Provide neutral head status if not enabled
                                head_status = {"drowsiness": "none", "is_nodding": False, "yaw": 0.0, "pitch": 0.0, "roll": 0.0}
                        except Exception as e:
